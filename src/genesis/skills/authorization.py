...
class SkillAuthorizationSnapshot(BaseModel):
    ...
    def from_backend(
        ...
        *,
        ...
        agent_skill_refs: Sequence[SkillReference],
        agent_permission_refs: Sequence[str],
        agent_scope_refs: Sequence[str],
        agent_allowed_tool_ids: Sequence[str],
        ...
        return cls(
            ...
            permission_refs=tuple(
                sorted(set(context.permission_refs) & set(agent_permission_refs))
            ),
            scope_refs=tuple(sorted(set(context.scope_refs) & set(agent_scope_refs))),
            allowed_tool_ids=tuple(
                sorted(set(context.allowed_tool_ids) & set(agent_allowed_tool_ids))
            ),
        )
